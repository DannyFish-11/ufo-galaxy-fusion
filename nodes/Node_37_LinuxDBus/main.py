...
class LinuxDBusTools:
    ...
    def get_tools(self):
        return [{"name": "call_method", "description": "调用方法", "parameters": {'service': '服务', 'method': '方法'}}]
    async def call_tool(self, tool: str, params: dict):
        handler = getattr(self, f"_tool_{tool}", None)
        if not handler:
            raise ValueError(f"Unknown tool: {tool}")
        return await handler(params)
    async def _tool_call_method(self, params):
...
@app.get("/health")
async def health():
    return {"status": "healthy", "node_id": "37", "name": "LinuxDBus", "timestamp": datetime.now().isoformat()}
