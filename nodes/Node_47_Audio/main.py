...
class AudioTools:
    ...
    def get_tools(self):
        return [{"name": "record", "description": "录音", "parameters": {'duration': '时长'}}]
    async def call_tool(self, tool: str, params: dict):
        handler = getattr(self, f"_tool_{tool}", None)
        if not handler:
            raise ValueError(f"Unknown tool: {tool}")
        return await handler(params)
    async def _tool_record(self, params):
...
@app.get("/health")
async def health():
    return {"status": "healthy", "node_id": "47", "name": "Audio", "timestamp": datetime.now().isoformat()}
