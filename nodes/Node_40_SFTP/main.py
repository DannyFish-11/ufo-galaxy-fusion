...
class SFTPTools:
    ...
    def get_tools(self):
        return [{"name": "upload", "description": "上传文件", "parameters": {'local': '本地路径', 'remote': '远程路径'}}]
    async def call_tool(self, tool: str, params: dict):
        handler = getattr(self, f"_tool_{tool}", None)
        if not handler:
            raise ValueError(f"Unknown tool: {tool}")
        return await handler(params)
    async def _tool_upload(self, params):
...
@app.get("/health")
async def health():
    return {"status": "healthy", "node_id": "40", "name": "SFTP", "timestamp": datetime.now().isoformat()}
