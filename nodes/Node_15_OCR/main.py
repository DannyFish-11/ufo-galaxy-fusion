...
class OCRTools:
    ...
    def get_tools(self):
        return [{"name": "recognize", "description": "识别文字", "parameters": {'image_path': '图片路径'}}]
    async def call_tool(self, tool: str, params: dict):
        handler = getattr(self, f"_tool_{tool}", None)
        if not handler:
            raise ValueError(f"Unknown tool: {tool}")
        return await handler(params)
    async def _tool_recognize(self, params):
...
@app.get("/health")
async def health():
    return {"status": "healthy", "node_id": "15", "name": "OCR", "timestamp": datetime.now().isoformat()}
